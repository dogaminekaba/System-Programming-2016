...
void receiveFile(FILE* fp, int connfd);
...
void receiveFile(FILE* fp, int connfd)
{
	...
	snprintf(sendBuff, MAX_BUF_SIZE, "File is sending...\n");
	while(write(connfd, sendBuff, strlen(sendBuff)) <= 0);
	
	while(strstr(recvBuff,"eotdmk") == NULL)
	{
	...
	snprintf(sendBuff, MAX_BUF_SIZE, "File is sent.\n");
	write(connfd, sendBuff, strlen(sendBuff));
}
